...
def move_start_point(pizza_height: int, pizza_width: int, current_x: int, current_y: int):
    if current_x >= pizza_width:
        ...
        current_y += 1
        if current_y % 10 == 0:
            print(f"{(current_y / pizza_height * 100)} %")
    else:
        current_x += 1
...
def write_solution(filename, slices):
    f = open(filename, 'w')
    f.write(f"{len(slices)}\n")
    for s in slices:
        f.write(f"{s.start.y} {s.start.x} {s.end.y} {s.end.x}\n")
    f.close()


if __name__ == '__main__':
    instance = 'medium'
    p, L, H = utils.parse_file(instance+'.in', get_pizza)
    sol = solve_greedy(p, L, H)
    print(sol)
    print(score(sol))
    write_solution(instance+'.out', sol)